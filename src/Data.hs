module Data(Tree(..), Rose(..), toTree, toBush, inorder, countOdd) where
...
data FunList a b c = Done a b | FunList a b (b -> c)
...
inorder m (Node left a right) = pure Node <*> inorder m left <*> m a <*> inorder m right

countOdd :: Integer -> State Integer Bool
countOdd n | even n    = pure False
countOdd _             = state (\n -> (True, n + 1))
